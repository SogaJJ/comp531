...
describe('Validate ToDoItem', () => {
	...
	it('should display a single ToDo', () => {
		...
		const node = TestUtils.renderIntoDocument(
			<div>
				<ToDoItem text={someText} done={false} toggle={() => {}} remove={()=>{}}/>
			</div>)
		...
		expect(element.children[1].innerHTML).to.equal(someText)
	})
	...
	it('should toggle completed when clicked', () => {
		...
		const node = TestUtils.renderIntoDocument(
			<div>
				<ToDoItem text={"something"} done={toggled} toggle={()=>{toggled=!toggled}} remove={()=>{}}/>
			</div>)
		const element = findDOMNode(node).children[0]
	...
	it('should remove an item when clicked', () => {
		...
		const node = TestUtils.renderIntoDocument(
			<div>
				<ToDoItem text={"something"} done={false} toggle={()=>{}} remove={()=>{removed=!removed}}/>
			</div>)
		const element = findDOMNode(node).children[0]
	...
	it('should display a completed ToDo', () => {
		...
		const node = TestUtils.renderIntoDocument(
			<div>
				<ToDoItem text={"something"} done={true} toggle={()=>{}} remove={()=>{}}/>
			</div>)
		const element = findDOMNode(node).children[0]
