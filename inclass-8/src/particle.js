...
const update = ({acceleration, velocity, position, mass}, delta, canvas) => {
	...
	position[0] -= position[0] > canvas.width ? canvas.width : 0
	position[0] += position[0] < 0 ? canvas.width : 0
	position[1] -= position[1] > canvas.height ? canvas.height : 0
	position[1] += position[1] < 0 ? canvas.height : 0
