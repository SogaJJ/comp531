...
class ToDoItem extends React.Component {
    ...
    render() { return (
        <li>
            ...
            <span className={this.state.done ? "completed" : ""}> {this.props.text}
            </span>
...
class ToDos extends React.Component {
    ...
    constructor(props) {
        ...
        this.state = {
            todoItems: [
                ...
                {id:1, text:"This is another item"}
            ]
        }
    }
    ...
    render() { return (
        <div>
            <input id="newTODO" type="text" placeholder="To Do" ref={ (node) => this.newTODO = node }>
            </input>
            <button onClick={() => this.addTodo()}> Add Item
            </button>
            <span className="submit">
                <a href="https://webdev-rice.herokuapp.com" target="_blank" >Submit your exercise
                </a>
            </span>
            <ul className="todo">
                {this.state.todoItems.map((x, item)=><ToDoItem key={x.id} text={x.text} remove={() => this.removeTodo(x.id) }/>)}
            </ul>
        </div>
